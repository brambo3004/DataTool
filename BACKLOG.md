...
1. **Duiding menselijker gemaakt**
   - Nieuwe kolommen `duiding_groep` en `duiding_uitleg` toegevoegd aan de Fase-4-actielijst.
   - Dezelfde duidingkolommen toegevoegd aan `Fase4_Mutatievoorstellen_[weg].csv`.
   - De app filtert de werkvoorraad nu op brede duidingsgroep in plaats van alleen op technische duidingscode.
   - De detailweergave toont een korte uitleg bij de duiding.
   - De veiligheidsregel blijft ongewijzigd: de tool signaleert en stelt voor, maar voert nooit automatisch wijzigingen door.

2. **Release-opbouw bevestigd**
   - De map `legacy/` blijft buiten de release-zip.
