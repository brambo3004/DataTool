...
def test_v0151_compact_route_start_outlier_uses_midpoint_in_overlap_cluster():
    """
    v0.15.1: een N354-achtige compacte groep met een route-start die kilometers
    terugschiet mag de Project Adviseur-volgorde niet terugtrekken.

    De ruwe route_start_m blijft zichtbaar voor controle, maar de sorteersleutel
    gebruikt route_mid_m omdat midden en einde wel logisch bij de buurgroep liggen.
    """
    items = [
        (
            "GRP_RIJBAAN_39",
            {
                ...
                "hm_min_sort": 29.5,
                "hm_max_sort": 29.5,
                "route_start_m": 44990.88,
                "route_mid_m": 44991.29,
                "route_end_m": 44995.54,
                "route_sort_m": 44990.88,
                "fallback_tie_breaker_dist": 0.0,
                "axis_tie_breaker_dist": 0.0,
        ...
        (
            "GRP_RIJBAAN_41",
            {
                ...
                "hm_min_sort": 29.6,
                "hm_max_sort": 30.0,
                "route_start_m": 45124.51,
                "route_mid_m": 45289.63,
                "route_end_m": 45473.74,
                "route_sort_m": 45124.51,
                "fallback_tie_breaker_dist": 0.0,
                "axis_tie_breaker_dist": 0.0,
                "sort_mode": "hm_overlap_route",
            },
    ...
    sorted_items = _sort_groups_with_overlap_clusters(items)
    data_by_id = dict(sorted_items)
    corrected = data_by_id["GRP_RIJBAAN_40"]

    assert [group_id for group_id, _ in sorted_items] == [
        "GRP_RIJBAAN_39",
        "GRP_RIJBAAN_40",
        "GRP_RIJBAAN_41",
    ]
    assert corrected["route_sort_m"] == 45101.60
    assert corrected["advisor_sort_m"] == 45101.60
    assert corrected["advisor_sort_raw_m"] == 39904.60
    assert corrected["route_sort_bron"] == "route_mid_m_gecorrigeerd_start_outlier"
    assert corrected["advisor_sort_correctie"] == "compacte_groep_route_start_outlier_gecorrigeerd"
