...
def _valid_hm_range(group_data: dict[str, Any]) -> tuple[float | None, float | None]:
    """Geef het geldige hm-bereik van een groep terug, of ``None`` bij ontbrekende data."""
    try:
        hm_min = float(group_data.get("hm_min_sort", group_data.get("sort_value", 99999.9)) or 99999.9)
...
def _route_conflict_sort_key(item: tuple[str, dict[str, Any]]) -> tuple[float, float, float, str]:
    """
    Sorteersleutel voor een routeconflictcluster.

    Binnen zo'n cluster spreken hm-volgorde en lokale route-as elkaar tegen. De
    lokale routepositie krijgt dan voorrang, maar hm_min en fallback blijven in
    de sleutel staan voor stabiliteit en reproduceerbaarheid.
    """
    group_id, group_data = item
    route_sort = _route_sort_for_conflict_resolution(group_data)
    hm_min, _ = _valid_hm_range(group_data)
    fallback = float(group_data.get("fallback_tie_breaker_dist", group_data.get("axis_tie_breaker_dist", 0.0)) or 0.0)

    return (
        route_sort if route_sort is not None else float("inf"),
        hm_min if hm_min is not None else float("inf"),
        fallback,
        group_id,
    )


def _mark_route_conflict_cluster(
    ...
    *,
    ...
    """
    Bewaar diagnosevelden voor groepen die door routeconflict-sortering zijn geraakt.
    """
    cluster_id = f"RC{rank}-{cluster_number}"
    cluster_size = len(cluster)

    for _, data in cluster:
        route_sort = _route_sort_for_conflict_resolution(data)
        fallback = float(data.get("fallback_tie_breaker_dist", data.get("axis_tie_breaker_dist", 0.0)) or 0.0)

        if data.get("route_sort_m") is None and route_sort is not None:
            _set_route_sort_metadata(
                data,
                route_sort_m=route_sort,
                route_sort_bron="route_conflict_m",
                fallback_sort_m=fallback,
            )

        data["route_conflict_cluster_id"] = cluster_id
        data["route_conflict_sort_applied"] = True
        data["route_conflict_cluster_size"] = cluster_size
        ...
        if str(data.get("sort_mode", "")).startswith("hm_overlap"):
            ...
            data["sort_mode"] = "hm_route_conflict"
        else:
            data["sort_mode"] = "hm_route_conflict"

        if data.get("tie_breaker_source") != "stabiele_fallback":
            data["tie_breaker_source"] = "lokale_route_as_conflictcluster"

        if route_sort is not None:
            data["tie_breaker_dist"] = float(route_sort)


def _resolve_route_backtracking_conflicts(
    sorted_items: list[tuple[str, dict[str, Any]]],
    ...
    clusters waar:
    - de routepositie terugloopt ten opzichte van een eerdere groep; én
    - de hm-bereiken elkaar overlappen of hetzelfde bereik raken.

    Daardoor veranderen we niet de hele wegvolgorde, maar alleen de stukken waar
    hm-volgorde en lokale route-as aantoonbaar met elkaar botsen.
    """
    if not sorted_items:
        return []
    ...
    for item in sorted_items:
        _, data = item
        items_by_rank[int(data.get("rank", 99))].append(item)

    for rank in sorted(items_by_rank):
        rank_items = items_by_rank[rank]
        i = 0
        conflict_cluster_counter = 0

        while i < len(rank_items):
            cluster = [rank_items[i]]
            cluster_has_conflict = False
            _, first_data = rank_items[i]
            cluster_route_max = _route_sort_for_conflict_resolution(first_data)
            hm_min, hm_max = _valid_hm_range(first_data)
            cluster_hm_max = hm_max if hm_max is not None else float("-inf")

            j = i + 1
            while j < len(rank_items):
                _, candidate_data = rank_items[j]
                candidate_route = _route_sort_for_conflict_resolution(candidate_data)
                candidate_hm_min, candidate_hm_max = _valid_hm_range(candidate_data)

                route_backtracks = (
                    cluster_route_max is not None
                    and candidate_route is not None
                    and candidate_route + 0.01 < cluster_route_max
                )
                hm_is_related = (
                    candidate_hm_min is not None
                    and candidate_hm_min <= cluster_hm_max + 0.0001
                )

                if route_backtracks and hm_is_related:
                    cluster_has_conflict = True
                    cluster.append(rank_items[j])
                    if candidate_route is not None:
                        cluster_route_max = max(cluster_route_max or candidate_route, candidate_route)
                    if candidate_hm_max is not None:
                        cluster_hm_max = max(cluster_hm_max, candidate_hm_max)
                    j += 1
                    continue

                # Als een conflictcluster eenmaal gestart is, nemen we direct
                # aansluitende/overlappende hm-groepen mee. Zo blijft het
                # kruispunt- of knipgebied als één lokaal sorteerprobleem bij elkaar.
                if cluster_has_conflict and hm_is_related:
                    cluster.append(rank_items[j])
                    if candidate_route is not None:
                        cluster_route_max = max(cluster_route_max or candidate_route, candidate_route)
                    if candidate_hm_max is not None:
            ...
            if cluster_has_conflict:
                conflict_cluster_counter += 1
                sorted_cluster = sorted(cluster, key=_route_conflict_sort_key)
                _mark_route_conflict_cluster(
                    sorted_cluster,
                    rank=rank,
                    cluster_number=conflict_cluster_counter,
                )
...
def _sort_groups_with_overlap_clusters(
    ...
    return _resolve_route_backtracking_conflicts(result)
