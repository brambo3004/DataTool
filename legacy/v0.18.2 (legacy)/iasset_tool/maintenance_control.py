...
def action_work_queue_summary(action_list: pd.DataFrame | None) -> dict[str, int]:
    """
    Tel de werkvoorraad voor het Fase-4-dashboard.

    De inhoudelijke Fase-4-controle blijft in `build_maintenance_control`.
...
def filter_action_work_queue(
    ...
    *,
    ...
    """
    Filter de Fase-4-werkvoorraad op veelgebruikte gebruikersfilters.
    ...
    records.append(
        {
            ...
            "ernst": ernst,
            "duiding": clean_display_value(duiding) or _duiding_for_suggestion_type(voorsteltype),
    ...
    if comparison.empty:
        ...
        summary = {
            ...
            "acties_waarschijnlijke_fout": int(
                action_list.get("duiding", pd.Series(dtype=str)).fillna("").astype(str).str.contains("waarschijnlijke_fout", regex=False).sum()
            ),
            "acties_twijfel_of_grensgeval": int(
                action_list.get("duiding", pd.Series(dtype=str)).fillna("").astype(str).str.contains("grensgeval", regex=False).sum()
            ),
            "acties_export_of_naamkwestie": int(
                action_list.get("duiding", pd.Series(dtype=str)).fillna("").astype(str).str.contains("export", regex=False).sum()
                + action_list.get("duiding", pd.Series(dtype=str)).fillna("").astype(str).str.contains("projectnaam", regex=False).sum()
            ),
        }
