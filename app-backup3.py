...
def load_data():
    ...
    # Hernoem bestaande ID kolommen om conflicten te voorkomen
    if 'id' in df.columns: 
        df.rename(columns={'id': 'bron_id'}, inplace=True)
    
    # Maak interne veilige ID
    ...
    # Zet index maar behoud kolom
    gdf.set_index('sys_id', drop=False, inplace=True)
    # BELANGRIJK: Verwijder de NAAM van de index om 'ValueError: cannot insert sys_id' in sjoin te voorkomen
    gdf.index.name = None
...
def build_graph_from_geometry(gdf):
    # Buffer iets ruimer (1.5m) om kiertjes tussen inritten en wegen te overbruggen
    gdf_buffer = gdf.copy()
    gdf_buffer['geometry'] = gdf_buffer.geometry.buffer(1.5)
    
    cols = ['geometry', 'subthema_clean', 'Rank', 'sys_id']
    
    # Maak kopieën voor sjoin en zorg dat de index geen naam heeft (conflicpreventie)
    ...
    # Filter self-loops op basis van de kolommen
...
def generate_grouped_proposals(gdf, G):
    """
    Nieuwe logica:
    1. Maak ruggengraat projecten (Rijbaan, Parallel, Fietspad).
    2. 'Absorbeer' secundaire objecten IN die projecten op basis van rank.
    """
    groups = {}
    node_to_group = {}
    ...
    # FASE 1: Ruggengraat (Segmentatie op lengte)
    ...
    while changes:
        ...
        for node_id in remaining:
            ...
            # Zoek buren die al in een groep zitten
            found_groups = set()
            for n in neighbors:
            ...
            if found_groups:
                ...
                # Rank 1 (Rijbaan) wint van Rank 2 (Parallel)
                candidates.sort(key=lambda x: x[0])
                best_group_id = candidates[0][1]
                
                # Absorbeer in dezelfde groep
...
if 'data_complete' not in st.session_state:
    ...
    # Check op sys_id om crash te voorkomen bij oude state
