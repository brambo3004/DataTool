...
def test_missing_project_violation_has_category_and_rule_code():
    gdf = _gdf([{"subthema": "rijstrook", "Onderhoudsproject": ""}])

    violation = check_rules(gdf)[0]

    assert violation["category"] == "Onderhoudsprojectplicht"
    assert violation["severity"] == "error"
    assert violation["rule_code"] == "MISSING_PROJECT"
    assert violation["issue_key"] == "MISSING_PROJECT:0"


def test_wrong_project_violation_has_own_category():
    gdf = _gdf([{"subthema": "perron", "Onderhoudsproject": "N398-HRB-01.2-03.4"}])
